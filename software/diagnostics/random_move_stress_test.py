...
def main():
    ...
    try:
        ...
        for i in range(REPEAT_COUNT):
            ...
            """
            极坐标法生成满足条件的x和y,无无效循环
            """
            # 1. 随机生成半径r（80 < r < 400）
            r = random.uniform(80, 400)
            # 2. 随机生成角度θ（0 到 π/2，保证x、y为正数）
            theta = random.uniform(-(3/4 * math.pi), (3/4 * math.pi))
            # 3. 极坐标转直角坐标
            x = r * math.cos(theta)
            y = r * math.sin(theta)
            z = random.uniform(0, 120)

            target_pos = [x, y, z, 0, 0, 0]
