...
本章说明如何使用项目已经提供的 Serial API、Python SDK、C++ SDK、Web、ROS 2、MoveIt 和 Gazebo 路径，把机械臂接入不同技术栈并跑起来。

第一次上手不需要先读本章；当你已经完成 [快速上手](../02_快速上手/README.md)，能够读取状态并让机械臂安全执行小动作后，再根据目标选择对应路径。

如果你的目标是修改源码、扩展 SDK、维护 ROS 2 硬件接口、调整 MoveIt/Gazebo 配置或开发 Web 控制端，请阅读 [开发者指南](../09_开发者指南/README.md)。本章优先回答“怎么用现有能力跑起来”，不展开“源码怎么改”。
...
| Web / URDF | 在浏览器里加载机械臂模型并做可视化或仿真交互 |
...
Serial API 是底层协议，Python SDK、C++ SDK、Web、ROS 2 和 LeRobot 都会以不同方式封装或复用它。使用上层框架前，建议先完成基础串口状态读取和小动作验证。
...
- [Web](Web/README.md)：Web 控制端、URDF 加载和网页仿真。
...
| 做网页可视化或 Web 仿真 | [Web](Web/README.md) |
...
| 修改 Web 项目结构或交互实现 | [Web 开发](../09_开发者指南/06_Web开发/README.md) |
| 扩展 LeRobot 插件 | [LeRobot 开发](../09_开发者指南/07_LeRobot开发/README.md) |
