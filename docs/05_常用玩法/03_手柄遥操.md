...
### Ubuntu 使用 Joy-Con 的依赖补充

如果你在 Ubuntu/Linux 上使用蓝牙 Joy-Con，并且系统蓝牙里已经显示连接成功，但脚本仍提示未检测到 Joy-Con，通常是 Python HID 后端没有使用 `hidraw`。

可以先安装编译依赖，并在当前 Python 环境中重新安装 `hidapi`：

```bash
sudo apt install build-essential python3-dev libudev-dev libusb-1.0-0-dev
python -m pip install --force-reinstall --no-binary=hidapi hidapi
```

安装后可以用下面的命令确认 `hidraw` 能看到 Joy-Con：

```bash
python -c "import hidraw; print([d for d in hidraw.enumerate() if 'Joy-Con' in (d.get('product_string') or '')])"
```

如果输出里能看到 `Joy-Con (R)` 或 `Joy-Con (L)`，说明 Joy-Con 已经能被 `hidraw` 识别。如果能识别但运行遥控脚本时报 `joycon connect failed`，通常是 `/dev/hidraw*` 权限不足，可以先用 `sudo` 运行脚本验证。
